...
export default function QuotationScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', description: '', refNumber: '' });
  ...
  const handleSave = async () => { if (!formData.customerName) { Alert.alert('خطأ', 'اختر العميل'); return; } await add({ ...formData, number: generateNumber(), totalAmount, items: lines.filter(l => l.itemName) }); setShowModal(false); };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <View style={st.h}><TouchableOpacity onPress={() => router.back()}><Text style={st.bt}>←</Text></TouchableOpacity><Text style={st.t}>عروض الأسعار</Text><TouchableOpacity style={st.ab} onPress={() => { setShowModal(true); }}><Text style={st.atx}>+</Text></TouchableOpacity></View>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '95%' }]}><View style={st.mh}><Text style={st.mt}>عرض سعر جديد</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
...
const st = StyleSheet.create({
  ...
  rc: { backgroundColor: '#16213E', borderRadius: 14, padding: 14, marginBottom: 10, marginHorizontal: 16, borderWidth: 1, borderColor: '#2a3550' }, rn: { color: '#D4AF37', fontSize: 14, fontWeight: 'bold' }, rd: { color: '#FFF', fontSize: 13 }, rt: { color: '#10B981', fontSize: 16, fontWeight: 'bold', marginTop: 4 },
