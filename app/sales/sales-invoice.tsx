...
export default function SalesInvoiceScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', warehouseId: '', warehouseName: '', cashBoxId: '', cashBoxName: '', paid: '0', discount: '0', description: '', refNumber: '' });
  ...
  const openEdit = (inv: any) => { setInvoiceType(inv.type); setFormData({ date: inv.date, customerId: inv.customerId || '', customerName: inv.customerName, warehouseId: inv.warehouseId || '', warehouseName: inv.warehouseName || '', cashBoxId: inv.cashBoxId || '', cashBoxName: inv.cashBoxName || '', paid: inv.paid?.toString() || '0', discount: inv.discount?.toString() || '0', description: inv.description || '', refNumber: inv.refNumber || '' }); setLines(inv.items?.length > 0 ? inv.items : [{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', freeQty: '0', price: '0', discount: '0', total: '0' }]); setSelectedInvoice(inv); setEditMode(true); setShowModal(true); };

  const filtered = invoices.filter((i: any) => i.number?.includes(searchQuery) || i.customerName?.includes(searchQuery));

  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <View style={st.h}><TouchableOpacity onPress={() => router.back()}><Text style={st.bt}>←</Text></TouchableOpacity><Text style={st.t}>فواتير المبيعات</Text><TouchableOpacity style={st.ab} onPress={() => { setEditMode(false); setSelectedInvoice(null); setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', warehouseId: '', warehouseName: '', cashBoxId: '', cashBoxName: '', paid: '0', discount: '0', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', freeQty: '0', price: '0', discount: '0', total: '0' }]); setShowModal(true); }}><Text style={st.atx}>+</Text></TouchableOpacity></View>
      ...
      {filtered.length === 0 ? <View style={st.e}><Text style={st.ei}>📄</Text><Text style={st.et}>لا توجد فواتير</Text></View> :
        <FlatList data={filtered} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.ic} onPress={() => openEdit(item)} onLongPress={() => Alert.alert('حذف', `حذف "${item.number}"؟`, [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            ...
            <Text style={st.icu}>👤 {item.customerName}</Text>
