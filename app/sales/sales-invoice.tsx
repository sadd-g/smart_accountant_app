...
interface SalesInvoice {
  id: string; number: string; date: string; customerName: string; subtotal: number;
  discount: number; tax: number; total: number; paid: number; remaining: number;
...
export default function SalesInvoiceScreen() {
  ...
  const { data: invoices, add, remove } = useLocalTable<SalesInvoice>('salesInvoices');
  ...
  const generateNumber = () => `SI-${(invoices.length + 1).toString().padStart(6, '0')}`;

  const handleSave = async () => {
    if (!customerName || lines.length === 0) { Alert.alert('خطأ', 'الرجاء إدخال العميل والأصناف'); return; }
    await add({
      number: generateNumber(), date, customerName, subtotal,
      discount: parseFloat(discount) || 0, tax: 0, total: totalAfterDiscount,
  ...
  const resetForm = () => {
    setDate(new Date().toISOString().split('T')[0]); setCustomerName(''); setPaymentType('cash');
    setPaid('0'); setDiscount('0'); setNotes('');
    setLines([{ id: '1', itemName: '', qty: '0', price: '0', total: '0' }]);
  };

  const filtered = invoices.filter((i: SalesInvoice) => 
    i.number?.includes(searchQuery) || i.customerName?.includes(searchQuery)
  );

  const totalSales = invoices.reduce((s: number, i: SalesInvoice) => s + (i.total || 0), 0);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>فواتير المبيعات ({invoices.length})</Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyIcon}>📄</Text><Text style={styles.emptyText}>لا توجد فواتير مبيعات</Text><Text style={styles.emptySubtext}>اضغط + لإضافة فاتورة</Text></View>
      ) : (
        <FlatList
          data={filtered}
          keyExtractor={(item: SalesInvoice) => item.id}
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.invoiceCard} onPress={() => { setSelectedInvoice(item); setShowDetailModal(true); }} onLongPress={() => handleDelete(item)}>
              ...
              <Text style={styles.invoiceCustomer}>👤 {item.customerName}</Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { maxHeight: '90%' }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>فاتورة مبيعات جديدة</Text>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <TextInput style={styles.fieldInput} value={customerName} onChangeText={setCustomerName} placeholder="اسم العميل" placeholderTextColor="#666" />
          ...
          <View style={styles.modalContent}>
            ...
            {selectedInvoice && (
              <ScrollView style={styles.modalBody}>
                ...
                <View style={styles.detailRow}><Text style={styles.detailLabel}>العميل</Text><Text style={styles.detailValue}>{selectedInvoice.customerName}</Text></View>
...
const styles = StyleSheet.create({
  ...
  invoiceCustomer: { color: '#FFFFFF', fontSize: 14, marginBottom: 6 },
