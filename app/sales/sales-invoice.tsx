  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>فواتير المبيعات</Text>
        <TouchableOpacity style={styles.addBtn} onPress={() => { setEditMode(false); setSelectedInvoice(null); setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', warehouseId: '', warehouseName: '', cashBoxId: '', cashBoxName: '', paid: '0', discount: '0', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', freeQty: '0', price: '0', discount: '0', total: '0' }]); setShowModal(true); }}>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { maxHeight: '95%' }]}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>{editMode ? 'تعديل' : 'إضافة'} فاتورة</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
      ...
      <PickerModal visible={showCustomerPicker} title="اختيار العميل" data={customers || []} displayField="name" subField="code" onSelect={(item) => setFormData({ ...formData, customerId: item.id, customerName: item.name })} onClose={() => setShowCustomerPicker(false)} />
      ...
      <PickerModal visible={showItemPicker} title="اختيار الصنف" data={items || []} displayField="name" subField="code" onSelect={(item) => { updateLine(currentLineId, 'itemId', item.id); updateLine(currentLineId, 'itemName', item.name); updateLine(currentLineId, 'price', item.salePrice?.toString() || '0'); updateLine(currentLineId, 'unit', item.unit || 'قطعة'); }} onClose={() => setShowItemPicker(false)} />
    </View>
  );
}
