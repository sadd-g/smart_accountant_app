...
export default function SalesInvoiceScreen() {
  ...
  const generateNumber = () => {
    ...
    const count = invoices.filter((i: SalesInvoice) => i.type === invoiceType).length;
  ...
  const filtered = invoices.filter((i: SalesInvoice) => i.number?.includes(searchQuery) || i.customerName?.includes(searchQuery));
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {filtered.length === 0 ? (
        ...
        <FlatList data={filtered} keyExtractor={(i: SalesInvoice) => i.id}
          renderItem={({ item }) => (
