...
export default function CustomersScreen() {
  ...
  const { data: customers, add } = useLocalTable('customers');
  ...
  const filtered = customers.filter((c: any) => (c.name || '').includes(searchQuery));
  const totalBalance = customers.reduce((s: number, c: any) => s + (c.balance || 0), 0);
  ...
  const handleSave = async () => {
    if (!formData.name) { Alert.alert('خطأ', 'أدخل اسم العميل'); return; }
    const exists = customers.find((c: any) => c.name === formData.name);
    if (exists) { Alert.alert('تنبيه', 'هذا العميل موجود بالفعل'); return; }
    
    await add({ ...formData, balance: parseFloat(formData.balance) || 0, creditLimit: parseFloat(formData.creditLimit) || 0 });
    const parentId = await getOrCreateParent('العملاء');
    ...
    Alert.alert('✅', 'تم إضافة العميل الفرعي برقم: ' + code);
  };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="العملاء" count={customers.length} onBack={() => router.back()} onAdd={() => { setFormData({ name: '', phone: '', address: '', currency: 'YER', balance: '0', creditLimit: '0' }); setShowModal(true); }} />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '75%' }]}><View style={st.mh}><Text style={st.mt}>إضافة عميل فرعي</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          <Text style={st.hint}>سيتم إضافته تحت حساب "العملاء" الرئيسي في الدليل</Text>
