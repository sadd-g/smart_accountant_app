...
export default function SalesReturnScreen() {
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="مرتجع مبيعات" count={returns.length} onBack={() => router.back()} onAdd={() => { setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', accountId: '', accountName: '', warehouseId: '', warehouseName: '', cashBoxId: '', cashBoxName: '', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', price: '0', total: '0' }]); setShowModal(true); }} />
      ...
      {returns.length === 0 ? <View style={st.e}><Text style={st.ei}>🔄</Text><Text style={st.et}>لا توجد مرتجعات</Text></View> :
        <FlatList data={returns} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc} onLongPress={() => Alert.alert('حذف', 'حذف؟', [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            <Text style={st.rn}>{item.number}</Text><Text style={st.rd}>👤 {item.customerName}</Text>
            <Text style={[st.rt, { color: item.type === 'cash' ? '#10B981' : '#F59E0B' }]}>{item.type === 'cash' ? '💰 نقدي' : '📋 آجل'} | {item.totalAmount?.toLocaleString()} ﷼</Text>
          </TouchableOpacity>
        )} contentContainerStyle={{ padding: 16 }} />}

      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '95%' }]}><View style={st.mh}><Text style={st.mt}>مرتجع مبيعات</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          {returnType === 'cash' && <>
            ...
            <TouchableOpacity style={st.pk} onPress={() => setShowCashBoxPicker(true)}><Text style={formData.cashBoxName ? st.pkt : st.pkp}>{formData.cashBoxName || 'اختيار الصندوق'}</Text><Text style={st.pka}>▼</Text></TouchableOpacity>
          </>}
          {returnType === 'credit' && <>
            <Text style={st.fl}>حساب العميل *</Text>
          ...
          <TouchableOpacity style={st.pk} onPress={() => setShowCustomerPicker(true)}><Text style={formData.customerName ? st.pkt : st.pkp}>{formData.customerName || 'اختيار العميل'}</Text><Text style={st.pka}>▼</Text></TouchableOpacity>
      ...
      <PickerModal visible={showCustomerPicker} title="اختيار العميل" data={customers || []} displayField="name" subField="code" onSelect={(i: any) => setFormData({ ...formData, customerId: i.id, customerName: i.name })} onClose={() => setShowCustomerPicker(false)} />
      ...
      <PickerModal visible={showItemPicker} title="اختيار الصنف" data={items || []} displayField="name" subField="code" onSelect={(i: any) => { updateLine(currentLineId, 'itemId', i.id); updateLine(currentLineId, 'itemName', i.name); updateLine(currentLineId, 'price', i.salePrice?.toString() || '0'); }} onClose={() => setShowItemPicker(false)} />
    </View>
  );
}
const st = StyleSheet.create({
  ...
  rc: { backgroundColor: '#16213E', borderRadius: 14, padding: 14, marginBottom: 10, marginHorizontal: 16, borderWidth: 1, borderColor: '#2a3550' }, rn: { color: '#D4AF37', fontSize: 14, fontWeight: 'bold' }, rd: { color: '#FFF', fontSize: 13 }, rt: { fontSize: 13, fontWeight: 'bold' },
