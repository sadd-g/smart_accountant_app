...
export default function SalesReturnScreen() {
  ...
  const { data: returns, add, remove } = useLocalTable('salesReturns');
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', warehouseId: '', warehouseName: '', description: '', refNumber: '' });
  ...
  const generateNumber = () => `SR-${(returns.length + 1).toString().padStart(6, '0')}`;

  const handleSave = async () => {
    if (!formData.customerName) { Alert.alert('خطأ', 'الرجاء اختيار العميل'); return; }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>مرتجع مبيعات</Text>
        <TouchableOpacity style={styles.addBtn} onPress={() => { setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', warehouseId: '', warehouseName: '', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', price: '0', total: '0' }]); setShowModal(true); }}>
      ...
      <View style={styles.controlBar}>
        ...
        <TouchableOpacity style={styles.printBtn}><Text>🖨️</Text></TouchableOpacity>
      </View>
      {returns.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyIcon}>🔄</Text><Text style={styles.emptyText}>لا توجد مرتجعات</Text></View>
      ) : (
        <FlatList data={returns} keyExtractor={(i: any) => i.id} renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onLongPress={() => Alert.alert('حذف', `حذف "${item.number}"؟`, [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            <Text style={styles.cardNumber}>{item.number}</Text><Text style={styles.cardCustomer}>👤 {item.customerName}</Text>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}><View style={styles.modalContent}>
          <View style={styles.modalHeader}><Text style={styles.modalTitle}>مرتجع مبيعات</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
          <ScrollView style={styles.modalBody}>
            <Text style={styles.fieldLabel}>رقم المرتجع</Text>
            <TextInput style={[styles.fieldInput, { color: '#D4AF37' }]} value={generateNumber()} editable={false} />
            <Text style={styles.fieldLabel}>العميل *</Text>
            <TouchableOpacity style={styles.pickerButton} onPress={() => setShowCustomerPicker(true)}>
              <Text style={formData.customerName ? styles.pickerText : styles.pickerPlaceholder}>{formData.customerName || 'اختيار العميل'}</Text><Text style={styles.pickerArrow}>▼</Text></TouchableOpacity>
      ...
      <PickerModal visible={showCustomerPicker} title="اختيار العميل" data={customers||[]} displayField="name" subField="code" onSelect={i=>setFormData({...formData,customerId:i.id,customerName:i.name})} onClose={()=>setShowCustomerPicker(false)} />
      ...
      <PickerModal visible={showItemPicker} title="اختيار الصنف" data={items||[]} displayField="name" subField="code" onSelect={i=>{updateLine(currentLineId,'itemId',i.id);updateLine(currentLineId,'itemName',i.name);}} onClose={()=>setShowItemPicker(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card:{backgroundColor:'#16213E',borderRadius:14,padding:14,marginBottom:10,borderWidth:1,borderColor:'#2a3550'},cardNumber:{color:'#D4AF37',fontSize:14,fontWeight:'bold'},cardCustomer:{color:'#FFFFFF',fontSize:13},cardTotal:{color:'#EF4444',fontSize:16,fontWeight:'bold',marginTop:4},
