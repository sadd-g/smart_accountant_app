...
export default function SalesReturnScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', warehouseId: '', warehouseName: '', description: '', refNumber: '' });
  ...
  const generateNumber = () => `PR-${(returns.length + 1).toString().padStart(6, '0')}`;

  const handleSave = async () => { if (!formData.supplierName) { Alert.alert('خطأ', 'اختر المورد'); return; } await add({ ...formData, number: generateNumber(), totalAmount, items: lines.filter(l => l.itemName) }); setShowModal(false); };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <View style={st.h}><TouchableOpacity onPress={() => router.back()}><Text style={st.bt}>←</Text></TouchableOpacity><Text style={st.t}>مردود مشتريات</Text><TouchableOpacity style={st.ab} onPress={() => { setShowModal(true); }}><Text style={st.atx}>+</Text></TouchableOpacity></View>
      ...
      {returns.length === 0 ? <View style={st.e}><Text style={st.ei}>🔄</Text><Text style={st.et}>لا توجد مرتجعات</Text></View> :
        <FlatList data={returns} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc} onLongPress={() => Alert.alert('حذف', 'حذف؟', [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            <Text style={st.rn}>{item.number}</Text><Text style={st.rd}>👤 {item.supplierName}</Text><Text style={st.rt}>{item.totalAmount?.toLocaleString()} ﷼</Text>
          </TouchableOpacity>
