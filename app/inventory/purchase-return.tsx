...
export default function SalesReturnScreen() {
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="مرتجع مشتريات" count={returns.length} onBack={() => router.back()} onAdd={() => { setFormData({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', accountId: '', accountName: '', warehouseId: '', warehouseName: '', cashBoxId: '', cashBoxName: '', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', price: '0', total: '0' }]); setShowModal(true); }} />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '95%' }]}><View style={st.mh}><Text style={st.mt}>مرتجع مشتريات</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          {returnType === 'credit' && <>
            <Text style={st.fl}>حساب المورد *</Text>
          ...
          <TouchableOpacity style={st.pk} onPress={() => setShowCustomerPicker(true)}><Text style={formData.supplierName ? st.pkt : st.pkp}>{formData.supplierName || 'اختيار المورد'}</Text><Text style={st.pka}>▼</Text></TouchableOpacity>
      ...
      <PickerModal visible={showCustomerPicker} title="اختيار المورد" data={suppliers || []} displayField="name" subField="code" onSelect={(i: any) => setFormData({ ...formData, supplierId: i.id, supplierName: i.name })} onClose={() => setShowCustomerPicker(false)} />
