...
interface Category { id: string; code: string; name: string; nameEn: string; }

export default function CategoriesScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { data: categories, add, remove } = useLocalTable<Category>('categories');
  const [searchQuery, setSearchQuery] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState({ name: '', nameEn: '' });

  const filtered = categories.filter((c: Category) => c.name?.includes(searchQuery));

  const handleSave = async () => {
    if (!formData.name) { Alert.alert('خطأ', 'الرجاء إدخال اسم الفئة'); return; }
    await add({ ...formData, code: formData.name.slice(0, 4).toUpperCase() });
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>الفئات ({categories.length})</Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyIcon}>🏷️</Text><Text style={styles.emptyText}>لا توجد فئات</Text></View>
      ) : (
        <FlatList data={filtered} keyExtractor={(i: Category) => i.id} renderItem={({ item }) => (
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>إضافة فئة</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
            <View style={styles.modalBody}>
              <Text style={styles.fieldLabel}>اسم الفئة *</Text>
              <TextInput style={styles.fieldInput} value={formData.name} onChangeText={(v) => setFormData({ ...formData, name: v })} placeholder="اسم الفئة" placeholderTextColor="#666" />
              <Text style={styles.fieldLabel}>الاسم بالإنجليزي</Text>
              <TextInput style={styles.fieldInput} value={formData.nameEn} onChangeText={(v) => setFormData({ ...formData, nameEn: v })} placeholder="Category name" placeholderTextColor="#666" />
...
const styles = StyleSheet.create({
  ...
  cardCode: { color: '#94a3b8', fontSize: 12 },
