...
export default function CategoriesScreen() {
  ...
  const { data: categories, add, remove } = useLocalTable('categories');
  const [searchQuery, setSearchQuery] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [newName, setNewName] = useState('');
  const filtered = (categories||[]).filter((c:any) => (c.name||'').includes(searchQuery));

  const handleAdd = async () => {
    if (!newName.trim()) { Alert.alert('خطأ', 'أدخل اسم الفئة'); return; }
    await add({ name: newName }); setNewName(''); setShowModal(false);
  };

  return (
    <View style={[st.c,{paddingTop:insets.top}]}><StatusBar barStyle="light-content"/>
      <View style={st.h}><TouchableOpacity onPress={()=>router.back()} style={st.b}><Text style={st.bt}>←</Text></TouchableOpacity><Text style={st.t}>🏷️ الفئات ({categories.length})</Text><TouchableOpacity style={st.ab} onPress={()=>setShowModal(true)}><Text style={st.at}>+</Text></TouchableOpacity></View>
      <TextInput style={st.si} placeholder="🔍 بحث..." placeholderTextColor="#94a3b8" value={searchQuery} onChangeText={setSearchQuery}/>
      {filtered.length===0?<View style={st.e}><Text style={st.ei}>🏷️</Text><Text style={st.et}>لا توجد فئات</Text></View>:
        <FlatList data={filtered} keyExtractor={(i:any)=>i.id} renderItem={({item}:any)=>(
          <TouchableOpacity style={st.rc} onLongPress={()=>Alert.alert('حذف',`حذف "${item.name}"؟`,[{text:'حذف',style:'destructive',onPress:()=>remove(item.id)},{text:'إلغاء'}])}>
            <Text style={st.rn}>🏷️ {item.name}</Text>
          </TouchableOpacity>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={st.mc}><View style={st.mh}><Text style={st.mt}>إضافة فئة</Text><TouchableOpacity onPress={()=>setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
          <View style={st.mb}><Text style={st.fl}>اسم الفئة</Text><TextInput style={st.fi} value={newName} onChangeText={setNewName} placeholder="اسم الفئة" placeholderTextColor="#666"/><TouchableOpacity style={st.sb} onPress={handleAdd}><Text style={st.sbt}>💾 حفظ</Text></TouchableOpacity></View>
        </View></View>
      </Modal>
    </View>
  );
