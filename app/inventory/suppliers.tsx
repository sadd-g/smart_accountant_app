...
export default function SupplieresScreen() {
  ...
  const [form, setForm] = useState({ name: '', name_ar: '', code: '', phone: 'c1', openingBalance: '' });

  useEffect(() => { if (db) loadData(); }, [db]);

  const loadData = async () => {
    if (!db) return;
    try { const rows = await db.getAllAsync("SELECT * FROM suppliers WHERE is_active=1 ORDER BY code"); setData(rows || []); } catch(e) {} finally { setLoading(false); }
  };
  ...
  const openAdd = () => { setEditId(null); setForm({ name: '', name_ar: '', code: '', phone: 'c1', openingBalance: '' }); setModalVisible(true); };
  const openEdit = (cb: any) => { setEditId(cb.id); setForm({ name: cb.name, name_ar: cb.name_ar, code: cb.code, phone: cb.currency_id||'c1', openingBalance: cb.opening_balance?.toString() || '' }); setModalVisible(true); };

  const handleSave = async () => {
    if (!form.name_ar) return Alert.alert('', 'الاسم مطلوب');
    if (!db) return;
    try {
      const ob = parseFloat(form.openingBalance) || 0;
      const code = form.code || 'CASH-' + (data.length + 1).toString().padStart(3, '0');
      if (editId) {
        await db.runAsync("UPDATE suppliers SET name=?, name_ar=?, code=?, currency_id=?, opening_balance=? WHERE id=?", [form.name, form.name_ar, code, form.phone, ob, editId]);
        Alert.alert('✅', 'تم التعديل');
      } else {
        await db.runAsync("INSERT INTO suppliers(id,code,name,name_ar,account_id,currency_id,opening_balance,current_balance) VALUES(?,?,?,?,?,?,?,?)", [genId(), code, form.name, form.name_ar, 'a1', form.phone, ob, ob]);
  ...
  const handleDelete = async (cb: any) => {
    Alert.alert('🗑️', `حذف "${cb.name_ar}"؟`, [{ text: 'إلغاء', style: 'cancel' }, { text: 'حذف', style: 'destructive', onPress: async () => { if (db) { await db.runAsync("UPDATE suppliers SET is_active=0 WHERE id=?", [cb.id]); await loadData(); } } }]);
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: '🏦 الموردين', headerStyle: { backgroundColor: '#FF9800' }, headerTintColor: '#fff' }} />
      <View style={[styles.topBar, { backgroundColor: colors.card }]}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={openAdd}><Ionicons name="add-circle" size={40} color="#FF9800" /></TouchableOpacity>
      </View>
      <FlatList data={filtered} keyExtractor={(cb: any) => cb.id} contentContainerStyle={{ padding: 12 }}
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, { backgroundColor: colors.card, borderLeftColor: '#FF9800' }]}>
            <TouchableOpacity style={{ flex: 1 }} onPress={() => openEdit(item)}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <Text style={[styles.balance, { color: '#FF9800' }]}>{formatNumber(item.current_balance || item.opening_balance || 0)}</Text>
              </View>
            </TouchableOpacity>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}><View style={[styles.modalContent, { backgroundColor: colors.background }]}>
          ...
          <ScrollView style={{ padding: 16 }}>
            ...
            <TextInput style={[styles.input, { backgroundColor: colors.card, color: colors.foreground, borderColor: colors.border }]} value={form.name_ar} onChangeText={v => setForm(f => ({ ...f, name_ar: v }))} placeholder="اسم الصندوق" placeholderTextColor={colors.mutedForeground} textAlign="right" />
