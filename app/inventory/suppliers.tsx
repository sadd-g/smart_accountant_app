...
export default function SuppliersScreen() {
  ...
  const filtered = suppliers.filter((c: any) => (c.name || '').includes(searchQuery));
  ...
  const handleSave = async () => {
    ...
    const parentId = await getOrCreateParent('الموردين');
    ...
    Alert.alert('✅', 'تم إضافة المورد الفرعي برقم: ' + code);
  };
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '75%' }]}><View style={st.mh}><Text style={st.mt}>إضافة مورد فرعي</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          <Text style={st.hint}>سيتم إضافته تحت حساب "الموردين" الرئيسي في الدليل</Text>
