...
export default function InventoryReceiptScreen() {
  ...
  const { data: receipts, add, remove, update } = useLocalTable('inventoryReceipts');
  ...
  const filtered = receipts.filter((r: any) => r.number?.includes(searchQuery));
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>توريد مخزون</Text>
        <TouchableOpacity style={styles.addBtn} onPress={() => { setFormData({ date: new Date().toISOString().split('T')[0], warehouseId: '', warehouseName: '', accountId: '', accountName: '', description: '', refNumber: '' }); setLines([{ id: '1', itemId: '', itemName: '', unit: 'قطعة', qty: '0', price: '0', total: '0' }]); setShowModal(true); }}>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyIcon}>📥</Text><Text style={styles.emptyText}>لا توجد عمليات توريد</Text></View>
      ) : (
        <FlatList data={filtered} keyExtractor={(i: any) => i.id} renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onLongPress={() => Alert.alert('حذف', `حذف "${item.number}"؟`, [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            ...
            <Text style={styles.cardDetail}>🏭 {item.warehouseName}</Text>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}><View style={styles.modalContent}>
          <View style={styles.modalHeader}><Text style={styles.modalTitle}>توريد مخزون</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
          <ScrollView style={styles.modalBody}>
            ...
            <Text style={styles.fieldLabel}>جهة التوريد</Text>
            <TouchableOpacity style={styles.pickerButton} onPress={() => setShowAccountPicker(true)}>
              <Text style={formData.accountName ? styles.pickerText : styles.pickerPlaceholder}>{formData.accountName || 'اختيار الحساب'}</Text><Text style={styles.pickerArrow}>▼</Text></TouchableOpacity>
