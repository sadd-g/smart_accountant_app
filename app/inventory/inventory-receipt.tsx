...
export default function InventoryReceiptScreen() {
  ...
  const { data: receipts, add, remove } = useLocalTable('inventoryReceipts');
  const { data: warehouses } = useLocalTable('warehouses');
  const { data: items } = useLocalTable('items');
  const [searchQuery, setSearchQuery] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], warehouseId: '', warehouseName: '', description: '', refNumber: '' });
  ...
  const generateNumber = () => `REC-${(receipts.length + 1).toString().padStart(6, '0')}`;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}><TouchableOpacity onPress={() => router.back()}><Text style={styles.backBtn}>←</Text></TouchableOpacity><Text style={styles.title}>توريد مخزون</Text><TouchableOpacity style={styles.addBtn} onPress={() => { setShowModal(true); }}><Text style={styles.addBtnText}>+</Text></TouchableOpacity></View>
      <View style={styles.controlBar}><TextInput style={styles.searchInput} placeholder="🔍 بحث..." placeholderTextColor="#94a3b8" value={searchQuery} onChangeText={setSearchQuery} /></View>
      {receipts.length === 0 ? (<View style={styles.empty}><Text style={styles.emptyIcon}>📥</Text><Text style={styles.emptyText}>لا توجد عمليات</Text></View>) : (
        <FlatList data={receipts} keyExtractor={(i: any) => i.id} renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onLongPress={() => Alert.alert('حذف', 'حذف؟', [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            <Text style={styles.cardNumber}>{item.number}</Text><Text style={styles.cardDetail}>🏭 {item.warehouseName}</Text><Text style={styles.cardTotal}>{item.totalAmount?.toLocaleString()} ﷼</Text>
          </TouchableOpacity>
        )} contentContainerStyle={{ padding: 16 }} />
      )}
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}><View style={styles.modalContent}><View style={styles.modalHeader}><Text style={styles.modalTitle}>توريد مخزون</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
        <ScrollView style={styles.modalBody}>
          ...
          <Text style={styles.fieldLabel}>المخزن</Text><TextInput style={styles.fieldInput} value={formData.warehouseName} onChangeText={v => setFormData({...formData, warehouseName: v})} placeholder="اسم المخزن" placeholderTextColor="#666" />
          ...
          {lines.map((line, i) => (
            <View key={line.id} style={styles.lineCard}>
              ...
              <TextInput style={styles.fieldInput} value={line.itemName} onChangeText={v => { const ll = [...lines]; ll[i].itemName = v; setLines(ll); }} placeholder="اسم الصنف" placeholderTextColor="#666" />
              <View style={styles.row}>
                <TextInput style={[styles.fieldInput, styles.half]} value={line.qty} onChangeText={v => { const ll = [...lines]; ll[i].qty = v; ll[i].total = (parseFloat(v||'0')*parseFloat(ll[i].price||'0')).toString(); setLines(ll); }} placeholder="كمية" placeholderTextColor="#666" keyboardType="numeric" />
                <TextInput style={[styles.fieldInput, styles.half]} value={line.price} onChangeText={v => { const ll = [...lines]; ll[i].price = v; ll[i].total = (parseFloat(v||'0')*parseFloat(ll[i].qty||'0')).toString(); setLines(ll); }} placeholder="سعر" placeholderTextColor="#666" keyboardType="numeric" />
              </View>
...
const styles = StyleSheet.create({
  ...
  controlBar:{flexDirection:'row',paddingHorizontal:16,marginBottom:12},searchInput:{flex:1,backgroundColor:'#16213E',borderRadius:10,padding:10,color:'#FFFFFF',borderWidth:1,borderColor:'#2a3550',textAlign:'right',fontSize:14},
  ...
  card:{backgroundColor:'#16213E',borderRadius:14,padding:14,marginBottom:10,borderWidth:1,borderColor:'#2a3550'},cardNumber:{color:'#D4AF37',fontSize:14,fontWeight:'bold'},cardDetail:{color:'#FFFFFF',fontSize:13},cardTotal:{color:'#10B981',fontSize:16,fontWeight:'bold',marginTop:4},
