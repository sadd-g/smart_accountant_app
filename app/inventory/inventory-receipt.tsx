...
export default function InventoryReceiptScreen() {
  ...
  const handleSave = async () => { if (!formData.warehouseName) { Alert.alert('خطأ', 'اختر المخزن'); return; } await add({ ...formData, number: generateNumber(), totalAmount, items: lines.filter(l => l.itemName) }); setShowModal(false); };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <View style={st.h}><TouchableOpacity onPress={() => router.back()}><Text style={st.bt}>←</Text></TouchableOpacity><Text style={st.t}>توريد مخزون</Text><TouchableOpacity style={st.ab} onPress={() => { setShowModal(true); }}><Text style={st.atx}>+</Text></TouchableOpacity></View>
      ...
      {receipts.length === 0 ? <View style={st.e}><Text style={st.ei}>📥</Text><Text style={st.et}>لا توجد عمليات</Text></View> :
        <FlatList data={receipts} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc} onLongPress={() => Alert.alert('حذف', 'حذف؟', [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }])}>
            <Text style={st.rn}>{item.number}</Text><Text style={st.rd}>🏭 {item.warehouseName} ← {item.accountName}</Text><Text style={[st.rt, { color: '#10B981' }]}>{item.totalAmount?.toLocaleString()} ﷼</Text>
          </TouchableOpacity>
        )} contentContainerStyle={{ padding: 16 }} />}
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '95%' }]}><View style={st.mh}><Text style={st.mt}>توريد مخزون</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
          <ScrollView style={st.mb}>
            ...
            <Text style={st.fl}>جهة التوريد</Text><TouchableOpacity style={st.pk} onPress={() => setShowAccountPicker(true)}><Text style={formData.accountName ? st.pkt : st.pkp}>{formData.accountName || 'اختيار الحساب'}</Text><Text style={st.pka}>▼</Text></TouchableOpacity>
            ...
            {lines.map((line, i) => (
              <View key={line.id} style={st.lc}>
                ...
                <Text style={[st.lt, { color: '#10B981' }]}>{parseFloat(line.total || '0').toLocaleString()} ﷼</Text>
              </View>
            ))}
...
const st = StyleSheet.create({
  ...
  rc: { backgroundColor: '#16213E', borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#2a3550' }, rn: { color: '#D4AF37', fontSize: 14, fontWeight: 'bold' }, rd: { color: '#FFF', fontSize: 13 }, rt: { fontSize: 16, fontWeight: 'bold', marginTop: 4 },
