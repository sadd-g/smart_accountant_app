...
interface Unit { id: string; code: string; name: string; nameEn: string; }

export default function UnitsScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { data: units, add, remove } = useLocalTable<Unit>('units');
  const [searchQuery, setSearchQuery] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState({ name: '', nameEn: '' });

  const filtered = units.filter((u: Unit) => u.name?.includes(searchQuery) || u.code?.includes(searchQuery));

  const handleSave = async () => {
    if (!formData.name) { Alert.alert('خطأ', 'الرجاء إدخال اسم الوحدة'); return; }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>وحدات القياس ({units.length})</Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyIcon}>📐</Text><Text style={styles.emptyText}>لا توجد وحدات</Text></View>
      ) : (
        <FlatList data={filtered} keyExtractor={(i: Unit) => i.id} renderItem={({ item }) => (
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>إضافة وحدة</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
            <View style={styles.modalBody}>
              <Text style={styles.fieldLabel}>اسم الوحدة *</Text>
              <TextInput style={styles.fieldInput} value={formData.name} onChangeText={(v) => setFormData({ ...formData, name: v })} placeholder="قطعة، كيلو..." placeholderTextColor="#666" />
              <Text style={styles.fieldLabel}>الاسم بالإنجليزي</Text>
              <TextInput style={styles.fieldInput} value={formData.nameEn} onChangeText={(v) => setFormData({ ...formData, nameEn: v })} placeholder="Piece, Kg..." placeholderTextColor="#666" />
