...
export default function CashReceiptScreen() {
  ...
  const loadVouchers = async () => { setLoading(true); try { setVouchers(await db.getAllAsync("SELECT * FROM vouchers WHERE type='receipt' ORDER BY created_at DESC LIMIT 50") || []); } catch(e) {} finally { setLoading(false); } };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const num = 'RV-' + Date.now().toString().slice(-6);
      const amt = parseFloat(form.amount);
      await db.runAsync("INSERT INTO vouchers(id,number,type,date,account_id,account_name,amount,description,ref_id,payment_method) VALUES(?,?,?,?,?,?,?,?,?,?)", [genId(), num, 'receipt', form.date, form.accountId, form.accountName, amt, form.description || 'سند قبض', form.refNumber, 'cash']);
      // تحديث رصيد الصندوق
      ...
      Alert.alert('✅', 'تم حفظ سند القبض: ' + num);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: '✅ سند قبض نقدي', headerStyle: { backgroundColor: '#2E7D32' }, headerTintColor: '#fff' }} />
      <View style={[styles.tabBar, { backgroundColor: colors.card }]}>
        <TouchableOpacity style={[styles.tab, tab === 'new' && styles.tabActive]} onPress={() => setTab('new')}><Ionicons name="add-circle" size={18} color={tab === 'new' ? '#2E7D32' : colors.mutedForeground} /><Text style={[styles.tabText, { color: tab === 'new' ? '#2E7D32' : colors.mutedForeground }]}>سند جديد</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.tab, tab === 'list' && styles.tabActive]} onPress={() => setTab('list')}><Ionicons name="list" size={18} color={tab === 'list' ? '#2E7D32' : colors.mutedForeground} /><Text style={[styles.tabText, { color: tab === 'list' ? '#2E7D32' : colors.mutedForeground }]}>السجل ({vouchers.length})</Text></TouchableOpacity>
      </View>
      ...
      {tab === 'new' ? (
        <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
          ...
          <TextInput style={[styles.input, { backgroundColor: colors.card, color: colors.foreground, height: 60 }]} value={form.description} onChangeText={v => setForm(f => ({ ...f, description: v }))} placeholder="وصف عملية القبض" placeholderTextColor={colors.mutedForeground} multiline textAlign="right" />
          ...
          <TouchableOpacity style={[styles.saveBtn, { backgroundColor: '#2E7D32' }]} onPress={handleSave} disabled={saving}>
            {saving ? <ActivityIndicator color="#fff" /> : <Ionicons name="save" size={20} color="#fff" />}
            <Text style={styles.saveText}>{saving ? 'جاري الحفظ...' : 'حفظ سند القبض'}</Text>
          </TouchableOpacity>
          <View style={{ height: 30 }} />
        </ScrollView>
      ) : (
        <FlatList data={vouchers} keyExtractor={v => v.id} contentContainerStyle={{ padding: 12 }}
          ListEmptyComponent={<View style={styles.empty}><Ionicons name="receipt-outline" size={50} color="#ccc" /><Text style={{ color: '#888', marginTop: 12 }}>لا توجد سندات</Text></View>}
          renderItem={({ item }) => (
            <View style={[styles.vCard, { backgroundColor: colors.card, borderLeftColor: '#2E7D32' }]}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                <Text style={[styles.vNum, { color: '#2E7D32' }]}>{item.number}</Text>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ color: '#2E7D32', fontSize: 18, fontWeight: '800' }}>{formatNumber(item.amount)}</Text>
          ...
          <FlatList data={filteredAccounts} keyExtractor={(a: any) => a.id} style={{ maxHeight: 350 }}
            renderItem={({ item }) => (
              <TouchableOpacity style={[styles.accItem, { borderBottomColor: colors.border }]} onPress={() => { setForm(f => ({ ...f, accountId: item.id, accountName: item.nameAr })); setShowAccountPicker(false); }}>
                <Text style={[styles.accCode, { color: '#2E7D32' }]}>{item.code}</Text>
...
const styles = StyleSheet.create({
  ...
  tabActive: { borderBottomWidth: 2, borderBottomColor: '#2E7D32' }, tabText: { fontSize: 14, fontWeight: '600' },
  ...
  accCode: { fontSize: 12, fontWeight: '700', backgroundColor: '#E8F5E9', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
});
