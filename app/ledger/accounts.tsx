...
export default function AccountsScreen() {
  ...
  const generateCode = (): string => {
    if (isSubAccount && parentAccount) {
      ...
      return parentAccount.code + (siblings.length + 1).toString().padStart(2, '0');
    }
  ...
  const handleSave = async () => {
    ...
    const parentId = isSubAccount ? parentAccount?.id || '' : '';
  ...
  const openAddSub = (account: any) => { 
    ...
    setParentAccount(account); setIsSubAccount(true); setEditMode(false); setSelectedAccount(null); setShowModal(true); 
  };
  ...
  filtered.forEach((acc: any) => {
    if (!acc.parentId) { 
      ...
      getSubAccounts(acc.id).filter((s: any) => (s.name || '').includes(searchQuery)).forEach((s: any) => { 
        displayAccounts.push({ ...s, _isSub: true }); addedSubs.add(s.id);
        getSubAccounts(s.id).filter((ss: any) => (ss.name || '').includes(searchQuery)).forEach((ss: any) => {
          displayAccounts.push({ ...ss, _isSub: true, _level: 2 }); addedSubs.add(ss.id);
        });
      }); 
    ...
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      {filtered.length === 0 ? <View style={st.e}><Text style={st.ei}>📚</Text><Text style={st.et}>لا توجد حسابات</Text></View> :
        <FlatList data={displayAccounts} keyExtractor={(i: any) => i.id + (i._isSub ? '_sub' : '')}
          renderItem={({ item }: any) => {
            const level = item._level || (item._isSub ? 1 : 0);
            return (
            <View style={{ marginLeft: level * 20 }}>
              <TouchableOpacity style={[st.rc, item._isSub && st.subRc]} onPress={() => { setFormData({ name: item.name, code: item.code, type: item.type, currency: item.currency || 'YER', balance: item.balance?.toString() || '0', parentId: item.parentId || '' }); setSelectedAccount(item); setIsSubAccount(!!item.parentId); if (item.parentId) setParentAccount(accounts.find((a: any) => a.id === item.parentId)); setEditMode(true); setShowModal(true); }} onLongPress={() => { const subs = getSubAccounts(item.id); if (subs.length > 0) { Alert.alert('تنبيه', 'لا يمكن حذف حساب يحتوي على حسابات فرعية'); return; } Alert.alert('حذف', `حذف "${item.name}"؟`, [{ text: 'حذف', style: 'destructive', onPress: () => remove(item.id) }, { text: 'إلغاء' }]); }}>
                <View style={st.rh}><View style={{ flex: 1 }}><Text style={st.rcode}>{item.code}</Text><Text style={st.rn}>{level > 0 ? '└ '.repeat(level) : ''}{item.name}</Text></View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    <View style={[st.tb, { backgroundColor: getTypeColor(item.type) + '20' }]}><Text style={[st.tt, { color: getTypeColor(item.type) }]}>{item.type}</Text></View>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '75%' }]}><View style={st.mh}><Text style={st.mt}>{editMode ? 'تعديل حساب' : (isSubAccount ? `فرعي لـ ${parentAccount?.name}` : 'حساب جديد')}</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          <TouchableOpacity style={st.pk} onPress={() => setShowCurrencyPicker(true)}><Text style={st.pkt}>{(currencies || []).find((c: any) => c.code === formData.currency)?.name || formData.currency}</Text><Text style={st.pka}>▼</Text></TouchableOpacity>
