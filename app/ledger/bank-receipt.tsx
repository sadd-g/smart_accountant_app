...
export default function CashReceiptScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.runAsync("INSERT INTO vouchers(id,number,type,date,account_id,account_name,amount,description,ref_id,payment_method) VALUES(?,?,?,?,?,?,?,?,?,?)", [genId(), num, 'receipt', form.date, form.accountId, form.accountName, amt, form.description || 'سند قبض', form.refNumber, 'bank']);
      // تحديث رصيد البنك
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: '✅ سند قبض بنكي', headerStyle: { backgroundColor: '#2E7D32' }, headerTintColor: '#fff' }} />
